...
def test_observation_action_spaces(env, agent_0):
    for agent in env.agents:
        ...
        if not (
            ...
            warnings.warn("Action space for each agent probably should be gymnasium.spaces.box or gymnasium.spaces.discrete")
        ...
        if isinstance(env.action_space(agent), gymnasium.spaces.Box):
            ...
            if np.any(np.equal(env.action_space(agent).high, np.inf)):
                warnings.warn("Agent's maximum action space value is infinity. This is probably too high")
        ...
        if isinstance(env.observation_space(agent), gymnasium.spaces.Box):
            ...
            if np.any(np.equal(env.observation_space(agent).high, np.inf)):
                warnings.warn("Agent's maximum observation space value is infinity. This is probably too high")
